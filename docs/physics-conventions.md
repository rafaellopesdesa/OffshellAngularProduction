...
The POWHEG `ZZ` implementation has no native four-lepton-mass keyword. A local
helper applies $150\leq m_{4\ell}\leq3000$ GeV to the qqZZ hard-event LHE
stream after POWHEG and before Pythia, aligning it with the gg4l and VPolar
generation phase space. A post-shower ATLAS `FourLeptonInvMassFilter` is
deliberately not used. The same helper checks the native gg4l range before
showering.

PowhegControl starts EVNT jobs with an LHE stream of approximately $1.1N$
events. The qqZZ card doubles that stream before generation, providing roughly
$2.2N$ hard events as headroom for the active LHE filter. If fewer than the
requested $N$ events survive, the job fails instead of publishing a truncated
sample.
